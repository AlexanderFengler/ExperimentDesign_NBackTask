function N_back_task_training(s_number, dif_lvl, trials)
...
for i = 1:trials
    i_num = num2str(i);
fid = ['color_distributions\n_back_color_distributions_' dif_lvl_str '_' color_num '.txt'];
...
f_name =['n_back_data\nback_subject_' string_s_number '_lvl_' string_dif_lvl '.txt'];
...
fprintf(f_curr, '%20s %3.0f\n\n','N-Back Difficulty: ',dif_lvl);
fprintf(f_curr, '%10s %10s %10s %10s %10s \n', 'Trial', 'Stimulus_Nr', 'Match' , 'Lure' , 'Correct');
...
fprintf(f_curr, '%10.0f %10.0f %10.0f %10.0f', trial, stimulus_nr, match , lure);
...
%Message indicating end of test
end_text = ['END of difficulty Lvl: ' string_dif_lvl];
